...
export default class CommandDispatcher<S> {
    ...
    public async execute(input: string | StringReader | ParseResults<S>, source: S = null): Promise<number> {
        ...
        if (input instanceof StringReader) {
            if (!(source == null))
                parse = await this.parse(input, source);
        } else
			parse = input;
    ...
    public parse(command: string | StringReader, source: S): Promise<ParseResults<S>> {
        ...
        for (let child of node.getRelevantNodes(originalReader)) {
            ...
            if (reader.canRead(child.getRedirect() == null ? 2 : 1)) {
                ...
                if (!(child.getRedirect() == null)) {
					...
					let parse: ParseResults<S> = await this.parseNodes(child.getRedirect(), reader, childContext);
					context.withChild(parse.getContext());
					return new ParseResults(context, parse.getReader(), parse.getExceptions());
				}
				else {
					let parse: ParseResults<S> = await this.parseNodes(child, reader, context);
					if (potentials == null) {
						potentials = [];
					}
    ...
    public async getSmartUsage(node: CommandNode<S>, source: S): Promise<Map<CommandNode<S>, string>> {
    ...
    private async __getSmartUsage(node: CommandNode<S>, source: S, optional: boolean, deep: boolean): Promise<string> {
        if (!await node.canUse(source)) {
            return null;
        }

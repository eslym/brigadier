...
export default class RequiredArgumentBuilder<S, T> extends ArgumentBuilder<S, RequiredArgumentBuilder<S, T>> {
    ...
    public static argument<S, T>(name: string, type: ArgumentType<T>): RequiredArgumentBuilder<S, T> {
    ...
    public build(): ArgumentCommandNode<S, T> {
        let result: ArgumentCommandNode<S, T> = new ArgumentCommandNode(this.getName(), this.getType(), this.getCommand(), this.getRequirement(), this.getRedirect(), this.getRedirectModifier(), this.isFork(), this.getSuggestionsProvider());

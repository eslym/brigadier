...
describe("Command Suggestion Test", () => {
	...
	async function testSuggestions(contents: string, cursor: number, range: StringRange, ...suggestions: string[]) {
        const result = await subject.getCompletionSuggestions(await subject.parse(contents, source), cursor);
    ...
    it('getCompletionSuggestions_rootCommands', async () => {
        ...
        const result = await subject.getCompletionSuggestions(await subject.parse("", source));

        expect(result.getRange()).to.deep.equal(StringRange.at(0));
        expect(result.getList()).to.have.deep.members([new Suggestion(StringRange.at(0), "bar"), new Suggestion(StringRange.at(0), "baz"), new Suggestion(StringRange.at(0), "foo")]);
    })
    ...
    it('getCompletionSuggestions_rootCommands_withInputOffset', async () => {
        ...
        const result = await subject.getCompletionSuggestions(await subject.parse(inputWithOffset("OOO", 3), source));

        expect(result.getRange()).to.deep.equal(StringRange.at(3));
        expect(result.getList()).to.have.deep.members([new Suggestion(StringRange.at(3), "bar"), new Suggestion(StringRange.at(3), "baz"), new Suggestion(StringRange.at(3), "foo")]);
    })
    ...
    it('getCompletionSuggestions_rootCommands_partial', async () => {
        ...
        const result = await subject.getCompletionSuggestions(await subject.parse("b", source));

        expect(result.getRange()).to.deep.equal(StringRange.between(0, 1));
        expect(result.getList()).to.have.deep.members([new Suggestion(StringRange.between(0, 1), "bar"), new Suggestion(StringRange.between(0, 1), "baz")]);
    })
    ...
    it('getCompletionSuggestions_rootCommands_partial_withInputOffset', async () => {
        ...
        const result = await subject.getCompletionSuggestions(await subject.parse(inputWithOffset("Zb", 1), source));

        expect(result.getRange()).to.deep.equal(StringRange.between(1, 2));
        expect(result.getList()).to.have.deep.members([new Suggestion(StringRange.between(1, 2), "bar"), new Suggestion(StringRange.between(1, 2), "baz")]);
    })
    ...
    it('getCompletionSuggestions_subCommands', async () => {
        ...
        const result = await subject.getCompletionSuggestions(await subject.parse("parent ", source));
    ...
    it('getCompletionSuggestions_subCommands_partial', async () => {
        ...
        const parse = await subject.parse("parent b", source);
        const result = await subject.getCompletionSuggestions(parse);

        expect(result.getRange()).to.deep.equal(StringRange.between(7, 8));
        expect(result.getList()).to.have.deep.members([new Suggestion(StringRange.between(7, 8), "bar"), new Suggestion(StringRange.between(7, 8), "baz")]);
    })
    ...
    it('getCompletionSuggestions_subCommands_partial_withInputOffset', async () => {
        ...
        const parse = await subject.parse(inputWithOffset("junk parent b", 5), source);
        const result = await subject.getCompletionSuggestions(parse);

        expect(result.getRange()).to.deep.equal(StringRange.between(12, 13));
        expect(result.getList()).to.have.deep.members([new Suggestion(StringRange.between(12, 13), "bar"), new Suggestion(StringRange.between(12, 13), "baz")]);
    })
    ...
    it('getCompletionSuggestions_redirect', async () => {
        ...
        const parse = await subject.parse("redirect ", source);
        const result = await subject.getCompletionSuggestions(parse);
    ...
    it('getCompletionSuggestions_redirectPartial', async () => {
        ...
        const parse = await subject.parse("redirect s", source);
        const result = await subject.getCompletionSuggestions(parse);
    ...
    it('getCompletionSuggestions_redirectPartial_withInputOffset', async () => {
        ...
        const parse = await subject.parse(inputWithOffset("/redirect s", 1), source);
        const result = await subject.getCompletionSuggestions(parse);
    ...
    it('getCompletionSuggestions_redirect_lots', async () => {
        ...
        const result = await subject.getCompletionSuggestions(await subject.parse("redirect loop 1 loop 02 loop 003 ", source));
    ...
    it('getCompletionSuggestions_execute_simulation', async () => {
        ...
        const parse = await subject.parse("execute as Dinnerbone as", source);
        const result = await subject.getCompletionSuggestions(parse);
    ...
    it('getCompletionSuggestions_execute_simulation_partial', async () => {
        ...
        const parse = await subject.parse("execute as bar as ", source);
        const result = await subject.getCompletionSuggestions(parse);

        expect(result.getRange()).to.deep.equal(StringRange.at(18));
        expect(result.getList()).to.have.deep.members([new Suggestion(StringRange.at(18), "bar"), new Suggestion(StringRange.at(18), "baz")]);
    })
})
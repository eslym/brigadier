...
describe('SuggestionTest', () => {
    ...
    it('expand_left', () => {
        const suggestion = new Suggestion(StringRange.at(1), "oo");
        assert.deepEqual(suggestion.expand("f", StringRange.between(0, 1)), new Suggestion(StringRange.between(0, 1), "foo"));
    })

    it('expand_right', () => {
        const suggestion = new Suggestion(StringRange.at(0), "minecraft:");
        assert.deepEqual(suggestion.expand("fish", StringRange.between(0, 4)), new Suggestion(StringRange.between(0, 4), "minecraft:fish"));
    })

    it('expand_both', () => {
        const suggestion = new Suggestion(StringRange.at(11), "minecraft:");
        assert.deepEqual(suggestion.expand("give Steve fish_block", StringRange.between(5, 21)), new Suggestion(StringRange.between(5, 21), "Steve minecraft:fish_block"));
    })

    it('expand_replacement', () => {
        const suggestion = new Suggestion(StringRange.between(6, 11), "strangers");
        assert.deepEqual(suggestion.expand("Hello world!", StringRange.between(0, 12)), new Suggestion(StringRange.between(0, 12), "Hello strangers!"));
    })
})
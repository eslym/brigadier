...
describe('SuggestionsBuilderTest', () => {
    ...
    it('suggest_multiple', () => {
        ...
        expect(result.getList()).to.have.deep.members([new Suggestion(StringRange.between(6, 7), "everybody"), new Suggestion(StringRange.between(6, 7), "weekend"), new Suggestion(StringRange.between(6, 7), "world!")]);
        assert.deepEqual(result.getRange(), StringRange.between(6, 7));
    ...
    it('sort_mixed', () => {
        const result = builder.suggest("11").suggest("22").suggest("33").suggest("a").suggest("b").suggest("c").suggest(2).suggest(4).suggest(6).suggest(8).suggest(30).suggest(32).suggest("3a").suggest("a3").build();
        const actual = result.getList().map(v => v.getText());
        expect(actual).to.have.deep.members([ "11", "2", "22", "33", "3a", "4", "6", "8", "30", "32", "a", "a3", "b", "c" ]);
    })
})
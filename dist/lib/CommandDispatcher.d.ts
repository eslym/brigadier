...
export default class CommandDispatcher<S> {
    ...
    execute(input: string | StringReader | ParseResults<S>, source?: S): Promise<number>;
    parse(command: string | StringReader, source: S): Promise<ParseResults<S>>;
    private parseNodes;
    getAllUsage(node: CommandNode<S>, source: S, restricted: boolean): string[];
    private __getAllUsage;
    getSmartUsage(node: CommandNode<S>, source: S): Promise<Map<CommandNode<S>, string>>;
